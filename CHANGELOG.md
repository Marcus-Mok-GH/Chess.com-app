...
### Performance
- Puzzle generation no longer freezes the app: the puzzles page now generates every lesson puzzle inside a web worker (`puzzleGenerator.worker.js`) through a promise-based client, with a synchronous fallback for browsers and test environments where workers are unavailable.
...
### Notes
- `pnpm run typecheck` for the api-server currently fails on `src/app.ts(6,29)` (missing declaration for `config/cors.js`). Verified this failure exists on `main` before this change and is unrelated to it. Frontend typecheck, full test suite (333/333), and client+SSR build are green.
...
### Added
- Gate-level regression tests (`PollinationsCoachGate.test.jsx`) covering anonymous users (prompt hidden), eligible logged-in users (prompt shown), and users who already saw the prompt (prompt hidden) — per CodeRabbit review on PR #168.
