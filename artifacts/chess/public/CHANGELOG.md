[2026-08-09] - Keep Play on setup and surface incomplete bot games on Home
...
- Added the API query and explicit local-game navigation needed to resume that game safely.
...
- Addressed CodeRabbit review feedback for the in-scope settings and navigation changes: restored `aria-pressed` on theme buttons, added an accessible `aria-label` to each `Toggle`, and added a `:focus-visible` outline on the volume slider.
- Apply the desktop `.app { height: 100dvh }` rule so the sidebar sits beside `main` and `main`'s `overflow-y: auto` provides an independent scroll area on desktop.
