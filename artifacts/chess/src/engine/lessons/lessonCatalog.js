 ...
 * Lessons Catalog
 *
 * Curated, structured learning content layered on top of the existing puzzle
 * engine. Each lesson targets one concrete concept, carries instructional
 * prose, an example position (FEN and/or PGN), and links to related puzzle
 * themes so a learner can practice the idea right after reading it.
 */

export const LESSON_DIFFICULTIES = ['beginner', 'intermediate'];
...
export function getLessonParagraphs(lesson) {
  if (!lesson) return [];
  if (Array.isArray(lesson.description)) return lesson.description;
  if (typeof lesson.description === 'string') return lesson.description.split(/\n\s*\n/).filter(Boolean);
  if (typeof lesson.content === 'string') return lesson.content.split(/\n\s*\n/).filter(Boolean);
  return [];
}
