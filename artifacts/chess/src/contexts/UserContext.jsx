...
export function UserProvider({ children }) {
    ...
    useEffect(() => {
        ...
        async function init() {
            ...
            const cachedUserRaw = (() => {
                try {
                    return localStorage.getItem(SESSION_USER_DATA_KEY);
                } catch {
                    return null;
                }
            })();
            const cacheEpoch = (() => {
                try {
                    return Number(
                        localStorage.getItem(SESSION_CACHE_EPOCH_KEY) || "0",
                    );
                } catch {
                    return 0;
                }
            ...
            const cacheFresh =
                cacheEpoch > 0 &&
                Date.now() - cacheEpoch < SESSION_CACHE_TTL_MS;
            ...
            try {
                ...
                console.warn(
                    "[UserContext] getSession threw (transient):",
                    e?.message || e,
                );
                if (isMounted) {
                    if (!cacheFresh) {
    ...
    const requestOtp = useCallback(async ({ email, username }) => {
        ...
        try {
            ...
            return {
                success: true,
                message:
                    "Code sent! Check your email for a 6-digit verification code.",
            };
        } catch (error) {
            localStorage.removeItem(PENDING_OTP_KEY);
