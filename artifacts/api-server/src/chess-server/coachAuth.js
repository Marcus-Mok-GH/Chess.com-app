...
function getAuthorizedCoachModels() {
  return (process.env.POLLINATIONS_COACH_MODELS || '')
    .split(',')
...
export async function createAuthorizationUrl(req) {
  ...
  const authorizedModels = getAuthorizedCoachModels();
  if (authorizedModels.length) authorizationUrl.searchParams.set('models', authorizedModels.join(','));
