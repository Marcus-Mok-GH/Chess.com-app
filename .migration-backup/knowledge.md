...
## Architecture
- **Client** (`src/`): App.jsx (routing), components/ (UI: ChessBoard.jsx etc.), pages/ (Home.jsx, Play.jsx), services/ (api.js, socket.js), engine/ (AI/engine)
- **Server** (`server/`): index.js (Express bootstrap), routes/ (users/games/engine), socket/ (matchmaking/game), db.js (Postgres pool)
...
- Reuse: chess.js lib, server-side Stockfish
